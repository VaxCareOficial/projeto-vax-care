...
function buscarSensoresExistentes(req, res){
    ...
    medidaModel.buscarSensoresExistentes().then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
...
function cadastrarMedidas(req,res){
    ...
    var temperatura = req.body.temperaturaServer;
        ...
        medidaModel.cadastrarMedidas(fkSensor, temperatura)
            .then(
                function (resultado) {
                    res.json(resultado);
                }
    ...
    function buscarDados(req, res){
        ...
        var idSensor = req.params.searchText;

        console.log(`Recuperando sensor por id`);
    
        medidaModel.buscarDados(idSensor, limite_linhas).then(function (resultado) {
            if (resultado.length > 0) {
                res.status(200).json(resultado);
            } else {
                res.status(204).send("Nenhum resultado encontrado!")
...
function buscarEmTempoReal(req, res) {
    ...
    medidaModel.buscarEmTempoReal(idSensor).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
